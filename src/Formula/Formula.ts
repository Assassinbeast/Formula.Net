namespace ff
{
	...
	export namespace Engine
	{
		export class CoreManager
		{
			...
			start()
			{
				...
				Utility.loadJavascriptFromRData(this.rData, () =>
				{
					var $appElement: HTMLElement = document.querySelector("ff-app");
					Utility.preProcessHtml($appElement);

					ff.coreMGR.viewControllerMGR.createApp($appElement);
					ff.coreMGR.viewControllerMGR.createViewControllers($appElement);
					ff.coreMGR.webobjectMGR.createWebObjects($appElement);

					ff.coreMGR.viewControllerMGR.startApp();
					ff.coreMGR.viewControllerMGR.startUnstartedViewControllers();
					ff.coreMGR.webobjectMGR.startUnstartedWebObjects();
						...
						for (var k = 0; k < removedNodes.length; k++)
						{
							...
							if (node.nodeType == node.ELEMENT_NODE)
							{
								...
								if ($element.getAttribute("ff-webobject"))
								{
									...
									ff.coreMGR.webobjectMGR.destroyWebObject(webobject);
								}
		...
		export class HistoryManager
		{
			...
			{
				(<events.IPrivateEventHandler><any>ff.events.onChangePageBegun).eventhandler.fireEvent(aTag.href);
				if (shallPushState)
			...
			private SPAPageChange2_ReceiveResponse(httpRequest: XMLHttpRequest, changePageId: number)
			{
				...
				ff.coreMGR.rData = Utility.createRDataObj(rDataDiv);
				if (ff.coreMGR.rData.ff_appversion !== ff.coreMGR.miscMGR.appVersion)
				{
					location.reload();
					return;
				}

				this.SPAPageChange3_LoadJavascript(httpRequest, $newViewCtrl, ff.coreMGR.rData, changePageId);
			}
			...
			private SPAPageChange5_UpdateNewHtmlDOM(httpRequest: XMLHttpRequest, $newViewCtrl: HTMLElement, rData: Items.RData)
			{
				...
				ff.coreMGR.viewControllerMGR.createViewControllers($newViewCtrl);
				ff.coreMGR.webobjectMGR.createWebObjects($newViewCtrl);
			...
			private SPAPageChange6_LastCode(httpRequest: XMLHttpRequest, rData: Items.RData, $rcFolder: HTMLElement, $newViewCtrl: HTMLElement)
			{
				...
				this.curShowingHistoryId = this.curHistoryId;
				(<events.IPrivateEventHandler><any>ff.events.onChangePageDone).eventhandler.fireEvent($newViewCtrl);
			...
			private getScrollYsObjects()
			{
				...
				addItem(ff.coreMGR.viewControllerMGR.$layout, "_layout");

				let $pages = ff.coreMGR.viewControllerMGR.$pages;
		...
		export class ScriptManager
		{
			...
			loadJavascripts(srcs: ScriptManager.ScriptFilePath[], onDone: (fails: ScriptManager.ScriptFilePath[]) => void)
			{
				var fails: ScriptManager.ScriptFilePath[] = [];
				var loadCount: number = 0;
				for (var i = 0; i < srcs.length; i++)
				{
					if (srcs[i].cdnPath in this.loadedJs ||
						srcs[i].path in this.loadedJs)
					{
						loadCount++;
						continue;
					}

					this.loadJavascript(srcs[i], 0, (success: boolean, src: ScriptManager.ScriptFilePath) =>
					{
						if (success == false)
							fails.push(src);
						loadCount++;
			...
			loadJavascript(src: ScriptManager.ScriptFilePath, index: number,
				onDone: (success: boolean, src: ScriptManager.ScriptFilePath) => void)
			{
				if (src.cdnPath)
				{
					this.loadJavascriptRaw(src.cdnPath, (success: boolean) =>
					{
						if (success == true)
							onDone(true, src);
						else if (src.path)
						{
							this.loadJavascriptRaw(src.path, (success2: boolean) =>
							{
								onDone(success2, src);
							});
						}
				...
				{
					this.loadJavascriptRaw(src.path, (success: boolean) =>
					{
						onDone(success, src);
					});
				}
			}
			loadJavascriptRaw(src: string, onDone:(success: boolean) => void)
			{
		...
		{
			export interface ScriptFilePath
			{
				cdnPath: string;
				path: string;
			}
		...
		export class ViewControllerManager
		{
			...
			createApp($appElement: HTMLElement)
			{
				var appObj = App;
				if (appObj == null)
					throw "The WebApplication must have an App class";

				this.app = new appObj();
				if (this.app instanceof BaseApp == false)
					throw "App class must be extended from th.BaseApp";

				this.app.$element = $appElement;
				this.app.$element["ff-ref"] = this.app;
				this.app.id = ff.coreMGR.miscMGR.createUId();
				if ("thAwake" in this.app)
					(<any>this.app).thAwake();
			}
			startApp()
			{
				if ("thStart" in this.app)
					(<any>this.app).thStart();
			}
			createViewControllers($parentElement: HTMLElement)
			{
				...
				if ($layout != null)
				{
					...
					if (classObj != null)
					{
						...
						layout.id = ff.coreMGR.miscMGR.createUId();
				...
				for (let i = 0; i < $pages.length; i++)
				{
					...
					if (classObj != null)
					{
						...
						page.$element = $page;
						page.id = ff.coreMGR.miscMGR.createUId();
						$page["ff-ref"] = page;
						if ("thAwake" in page)
							(<any>page).thAwake();
					}
		...
		export class WebObjectManager
		{
			...
			{ 
				...
				webobjectInstance.id = ff.coreMGR.miscMGR.createUId();
		...
		export class Utility
		{
			...
			static scrollToHash(hash: string)
			{
				...
				if (element == null)
					return;
				this.scrollToTarget(element, ff.coreMGR.miscMGR.hashScrollExtraSpace);
			}
			static scrollLayoutAndPageScrollYObjects(layoutAndPageScrollYObjects: any): void
			{
				...
				{
					for (let i = 0; i < ff.coreMGR.viewControllerMGR.pages.length; i++)
					{
						var page = ff.coreMGR.viewControllerMGR.pages[i];
				...
				for (let key in layoutAndPageScrollYObjects)
				{
					...
					{
						let $layout = ff.coreMGR.viewControllerMGR.$layout;
						let y = layoutAndPageScrollYObjects["_layout"];
						this.scrollElement($layout, y);
					}
					else //its a page
					{
						var $page = ff.coreMGR.viewControllerMGR.$pages[key];
						if ($page != null)
						{
			...
			static preProcessHtml($parentObj: HTMLElement)
			{
				...
				(<events.IPrivateEventHandler><any>ff.events.onInitObjects).eventhandler.fireEvent($parentObj);
			}
			static initATags($parentObj: HTMLElement)
			{
				var subscribeATagOnClick = function ($aTag: HTMLAnchorElement)
				{
					...
					$aTag.onclick = () =>
					{
						ff.coreMGR.historyMGR.linkClick($aTag.href);
						return false;
					}
			...
			static loadJavascriptFromRData(rData: Items.RData, onDone?: () => void)
			{
				var scripts = rData.ff_scripts;
				var srcs: ScriptManager.ScriptFilePath[] = [];
				for (let i = 0; i < scripts.length; i++)
				{
					srcs.push({
						cdnPath: coreMGR.miscMGR.cdn ? Helper.textTrimEnd(coreMGR.miscMGR.cdn, "/") + "/" +
							Helper.textTrimStart(scripts[i].path, "/") : null,
						path: scripts[i].path
					});
				}
				...
				ff.coreMGR.scriptMGR.loadJavascripts(srcs, (fails: ScriptManager.ScriptFilePath[]) =>
				{
					var fatalFails: ScriptManager.ScriptFilePath[] = [];
					//TODO: this can be make into dictionary in c#, so its faster than a list when searching
					for (let i = 0; i < fails.length; i++)
						for (let j = 0; j < scripts.length; j++)
						{
							if (scripts[j].path == fails[i].path &&
								scripts[j].isFatal)
							{
								fatalFails.push(fails[i]);
							}
						}
					if (fatalFails.length >= 1)
					{
						var errorText = "";
						for (let i = 0; i < fatalFails.length; i++)
						{
							if (fatalFails[i].cdnPath && fatalFails[i].path)
								errorText += fatalFails[i].cdnPath + " and " + window.location.origin + "/" + fatalFails[i].path + "\n";
							else if (fatalFails[i].cdnPath)
								errorText += fatalFails[i].cdnPath + "\n";
							else if (fatalFails[i].path)
								errorText += window.location.origin + "/" + fatalFails[i].path + "\n";
						}
						errorText = errorText.substr(0, errorText.length - 1);
						throw "The following javascript(s) was not able to load:\n\n" + errorText;
					}
		...
		export class Config {
			...
			get hashScrollExtraSpace(): number {
				return ff.coreMGR.miscMGR.hashScrollExtraSpace;
			}
			/**
			...
			set hashScrollExtraSpace(value: number) {
				ff.coreMGR.miscMGR.hashScrollExtraSpace = value;
			}
			/**
			...
			setProgressBar(progressBar: Items.IProgressBar) {
				ff.coreMGR.historyMGR.progressBar = progressBar;
			}
			/**
	...
	*/
	export var coreMGR: ff.Engine.CoreManager;
	/**
	The RData which contains many of the variables sent back from the server
	*/
	export function getRData(): ff.Items.RData
	{
	...
	export function start()
	{
		...
		coreMGR = new ff.Engine.CoreManager();
		coreMGR.start();
	}
	/**
	...
	{
		if (id == null)
			return ff.coreMGR.webobjectMGR.webobjects_id;
		if (typeof id == "string") {
			if (isNaN(parseInt(id)))
				return ff.coreMGR.webobjectMGR.webobjects_type[id];
			else
				return ff.coreMGR.webobjectMGR.webobjects_id[id];
		}
		else
			return ff.coreMGR.webobjectMGR.webobjects_id[id.toString()];
	}
	/**
	...
	{
		ff.coreMGR.historyMGR.linkClick(href);
	}
	/**
	...
	{
		if (data == null)
			return ff.coreMGR.historyMGR.getHistoryCustomData();
		else
			ff.coreMGR.historyMGR.setHistoryCustomData(data);
	}
	...
	{
		ff.Engine.Utility.preProcessHtml($parentObj);
		ff.coreMGR.webobjectMGR.createWebObjects($parentObj);
		ff.coreMGR.webobjectMGR.startUnstartedWebObjects();
